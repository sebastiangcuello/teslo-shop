...
export const placeOrder = async (
    ...
    const itemsInOrder = productsOrder.reduce((count, p) => count + p.quantity, 0);

    // Los totales de tax, subtotal, y total
    const { subTotal, tax, total } = productsOrder.reduce(
        (totals, item) => {
